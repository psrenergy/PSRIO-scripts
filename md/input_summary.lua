...
local function demand_chart(case, tab)
    ...
    -- local inelastic_demand = ifelse(demand.is_elastic:eq(0),demand_data,0);
    -- local elastic_demand = ifelse(demand.is_elastic:eq(1),demand_data,0);
    ...
    -- if elastic_demand:loaded() then
    --     local chart = Chart();
    --     chart:add_column(elastic_demand,{color = colors.elastic_demand});
    --     tab:push("## " .. dictionary.elastic_demand[language]);
    --     tab:push(chart);
    -- end

end

local function capacity_chart(case, tab)
    ...
    if some_data_loaded then
        local chart = Chart();
        chart:add_categories(thermal_capacity, dictionary.thermal_capacity[language], {xLabel  = dictionary.system[language],
        ...
        chart:add_categories(renewable_capacity,  dictionary.wind_capacity[language]);
