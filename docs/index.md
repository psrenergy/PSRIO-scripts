...
PSRIO is a scripting language based on LUA designed for managing data held in PSR databases. The idea is to query output data of PSR models, perform several user-specified mathematical, statistical, and data processing operations and finally produce new customized outputs in the same format as the SDDP outputs that can be processed by the graph tool Additionally, PSRIO also enables the direct creation of dashboard.
At the end of the day, by delegating this task to an easy-to-use and secure language, the main objective of this tool is to automate and standardize everyday operations, avoiding manual errors and repeated work. The “recipe files” are saved in the case directory and automatically processed after SDDP execution.

### LUA

![](https://upload.wikimedia.org/wikipedia/commons/thumb/c/cf/Lua-Logo.svg/1200px-Lua-Logo.svg.png | width=256)
